...
//  [ ] Platform: Multi-viewport: Missing ImGuiBackendFlags_HasParentViewport support. The
//  viewport->ParentViewportID field is ignored, and therefore io.ConfigViewportsNoDefaultParent has
//  no effect either.
...
//  2026-03-25: Mouse cursor is properly restored if changed by user app/code while using
//  glfwSetInputMode(..., GLFW_CURSOR_DISABLED) or ImGuiConfigFlags_NoMouseCursorChange. Amend
//  change from 2025-12-10. 2026-02-10: Try to set IMGUI_IMPL_GLFW_DISABLE_X11 /
//  IMGUI_IMPL_GLFW_DISABLE_WAYLAND automatically if corresponding headers are not accessible.
//  (#9225) 2026-01-25: [Docking] Improve workarounds for cases where GLFW is unable to provide any
//  reliable monitor info. Preserve existing monitor list when none of the new one is valid. (#9195,
//  #7902, #5683) 2026-01-18: [Docking] Dynamically load X11 functions to avoid -lx11 linking
//  requirement introduced on 2025-09-10. 2025-12-12: Added IMGUI_IMPL_GLFW_DISABLE_X11 /
...
//  are always reported as 1.0 on X11. (#8920, #8921) 2025-09-10: [Docking] Improve multi-viewport
//  behavior in tiling WMs on X11 via the ImGui_ImplGlfw_SetWindowFloating() function. Note: using
//  GLFW backend on Linux/BSD etc. requires linking with -lX11. (#8884, #8474, #8289) 2025-07-08:
//  Made ImGui_ImplGlfw_GetContentScaleForWindow(), ImGui_ImplGlfw_GetContentScaleForMonitor()
...
//  DPI-aware apps. 2025-05-15: [Docking] Add Platform_GetWindowFramebufferScale() handler, to allow
//  varying Retina display density on multiple monitors. 2025-04-26: [Docking] Disable
//  multi-viewports under Wayland. (#8587) 2025-03-10: Map GLFW_KEY_WORLD_1 and GLFW_KEY_WORLD_2
//  into ImGuiKey_Oem102. 2025-03-03: Fixed clipboard handler assertion when using GLFW <= 3.2.1
//  compiled with asserts enabled. 2025-02-21: [Docking] Update monitors and work areas information
//  every frame, as the later may change regardless of monitor changes. (#8415) 2024-11-05:
//  [Docking] Added Linux workaround for spurious mouse up events emitted while dragging and
//  creating new viewport. (#3158, #7733, #7922) 2024-08-22: Moved some OS/backend related function
...
//  #6096) 2023-01-18: Handle unsupported glfwGetVideoMode() call on e.g. Emscripten. 2023-01-04:
//  Inputs: Fixed mods state on Linux when using Alt-GR text input (e.g. German keyboard layout),
//  could lead to broken text input. Revert a 2022/01/17 change were we resumed using mods provided
//  by GLFW, turns out they were faulty. 2022-11-22: Perform a dummy glfwGetError() read to cancel
//  missing names with glfwGetKeyName(). (#5908) 2022-10-18: Perform a dummy glfwGetError() read to
//  cancel missing mouse cursors errors. Using GLFW_VERSION_COMBINED directly. (#5785) 2022-10-11:
//  Using 'nullptr' instead of 'NULL' as per our switch to C++11. 2022-09-26: Inputs: Renamed
//  ImGuiKey_ModXXX introduced in 1.87 to ImGuiMod_XXX (old names still supported). 2022-09-01:
//  Inputs: Honor GLFW_CURSOR_DISABLED by not setting mouse position *EDIT* Reverted 2023-07-18.
//  2022-04-30: Inputs: Fixed ImGui_ImplGlfw_TranslateUntranslatedKey() for lower case letters on
//  OSX. 2022-03-23: Inputs: Fixed a regression in 1.87 which resulted in keyboard modifiers events
...
void ImGui_ImplGlfw_MouseButtonCallback(GLFWwindow* window, int button, int action, int mods) {
    ...
    // Workaround for Linux: ignore mouse up events which are following an focus loss following a
    // viewport creation
...
void ImGui_ImplGlfw_WindowFocusCallback(GLFWwindow* window, int focused) {
    ...
    // Workaround for Linux: when losing focus with MouseIgnoreButtonUpWaitForFocusLoss set, we will
    // temporarily ignore subsequent Mouse Up events
...
void ImGui_ImplGlfw_MonitorCallback(GLFWmonitor*, int) {
    // This function is technically part of the API even if we stopped using the callback, so
    // leaving it around.
}
...
static bool ImGui_ImplGlfw_Init(GLFWwindow* window, bool install_callbacks,
    ...
    if (!bd->IsWayland) {
        ...
        bd->XChangeWindowAttributes =
            (PFN_XChangeWindowAttributes)dlsym(bd->X11Module, "XChangeWindowAttributes");
        bd->XFlush = (PFN_XFlush)dlsym(bd->X11Module, "XFlush");
        IM_ASSERT(bd->XInternAtom != nullptr && bd->XChangeProperty != nullptr &&
...
static void ImGui_ImplGlfw_UpdateMouseData() {
    ...
    for (int n = 0; n < platform_io.Viewports.Size; n++) {
        ...
        if (is_window_focused) {
            ...
            // io.ConfigNavMoveSetMousePos is enabled by user) When multi-viewports are enabled, all
            // Dear ImGui positions are same as OS positions.
        ...
        // _NoInputs flag (since we implement hit via our WndProc hook)
        //       On other platforms we rely on the library fallbacking to its own search when
        //       reporting a viewport with _NoInputs flag.
...
static void ImGui_ImplGlfw_SetWindowFloating(ImGui_ImplGlfw_Data* bd, GLFWwindow* window) {
    ...
    if (glfwGetPlatform() == GLFW_PLATFORM_X11) {
        ...
        bd->XChangeProperty(display, xwindow, wm_type, XA_ATOM, 32, PropModeReplace,
...
static LRESULT CALLBACK ImGui_ImplGlfw_WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    switch (msg) {
        ...
        // Add support for Win32 (based on imgui_impl_win32), because we rely on _TouchScreen info
        // to trickle inputs differently.
    case WM_MOUSEMOVE:
    case WM_NCMOUSEMOVE:
    case WM_LBUTTONDOWN:
