...
void Frustum::update(const Camera* cam, f32 v_fov) {
    path.clear(); 
    ...
    const f32 halfNearPlaneHeight = zNear * std::tan(glm::radians(v_fov) / 2.0f);
...
//    path.add(zNearTR).add(zNearBR).add(zNearBL).add(zNearTL).add(zNearTR);
    ...
    const f32 halfFarPlaneHeight = zFar * std::tan(glm::radians(v_fov) / 2.0f);
    ...
    this->near = Plane(pos + front * zNear, front,"near");
    this->far = Plane(pos + frontMulFar, -front,"far");

    this->right = Plane(pos, cross(frontMulFar - right * halfFarPlaneWidth, up),"right");
    this->left = Plane(pos, cross(up, frontMulFar + right * halfFarPlaneWidth),"left");

    this->top = Plane(pos, cross(right, frontMulFar - up * halfFarPlaneHeight),"top");

    this->bot = Plane(pos, cross(frontMulFar + up * halfFarPlaneHeight, right),"bot");
}
...
Frustum::Frustum(const Camera* cam, f32 v_fov) {
    update(cam,v_fov);
}
